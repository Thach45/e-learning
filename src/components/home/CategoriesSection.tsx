    ...
    <section className="py-6">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm">Khám phá các chủ đề phổ biến nhất hiện nay</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
        {categories.map(cat => (
          <a key={cat.id} href="#" className="group bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-indigo-100/30 hover:-translate-y-1 transition-all text-center">
            {cat.imageUrl ? (
              <img
                src={cat.imageUrl}
                alt={cat.name}
            ...
            <h3 className="font-bold text-slate-700 text-sm group-hover:text-indigo-600 transition-colors">{cat.name}</h3>
